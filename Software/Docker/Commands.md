...
## Personally Useful commands

`docker build [dockerfile] --tag [name of image]`
Build local Dockerfile and name it 
...
|[docker manifest](https://docs.docker.com/engine/reference/commandline/manifest/)|Manage [Docker image](https://www.knowledgehut.com/blog/devops/best-docker-books) manifests and manifest lists|
...
|[docker search](https://docs.docker.com/engine/reference/commandline/search/)|Search the [Docker Hub](https://www.knowledgehut.com/blog/devops/docker-images-command) for images|
|[docker secret](https://docs.docker.com/engine/reference/commandline/secret/)|Manage [Docker secrets](https://www.knowledgehut.com/blog/devops/docker-secrets)|
...
## Docker Compose 
Docker compose can either be used as a plugin to docker using `docker-compose` or as a standalone using `docker compose`. 

Great for orchestrating multiple services. Based on docker-compose build file.

`build`: Builds a docker image based on specs from the docker-compose.yaml.
`up`: Create and start containers. Can bring up individual or all containers.
`run`: Run a one off command in a docker container.  Only on one container.
